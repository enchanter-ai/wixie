...
| "Skip the lifecycle, just edit the file" | "OK, editing." | "File edit alone won't refresh the metadata. Next lifecycle verdict will see stale scores and re-fire. Edit anyway and accept the churn?" |
...
4. Let the user override. If they override with a reason, log the override per [`./discipline.md`](./discipline.md) § When to violate.
...
| Module | Role |
|---|---|
| [`./discipline.md`](./discipline.md) § Think-first | Surface assumptions *at the start* of work |
...
- If it's about *your* sycophancy or shallow framing: log to the project's precedent log with tag `doubt-engine-catch` and code F01.
- If it's a one-off operational gotcha: precedent log only.
- If it's a recurring pattern across sessions, promote it from precedent to a higher-tier learning surface per the project's accumulation contract.
