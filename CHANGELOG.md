...
- **`count-facts.py` — derive-from-source count guard** (`shared/scripts/count-facts.py`). Documented counts (the model count today; extensible to plugins/agents) are derived from the registry and either `check`ed (CI fails on drift) or `inject`ed (rewritten to match) across the README badge/anchor/heading/prose, `CLAUDE.md`, `CONTRIBUTING`, `CITATION.cff`, `marketplace.json`, `docs/`, and the mermaid diagrams. Ends the hand-copied-count drift that ran 64 → 274 → 447. Wired into CI via `tests/prompt-crafter/test-count-facts.sh`. `inject` also propagates the model count into adjacent sibling repos' org-profile READMEs (`../{crow,djinn,emu,hydra,lich,pech,sylph}/docs/org-profile-README.md`) when they are checked out next to wixie — one command fixes the whole org after a registry bump. Siblings are an inject-only target; wixie's own `check` never gates on another repo's docs.
